function CGame(oData, iLevel) {
    ...
    this.loseBall = function (iID) {
        ...
        var oSoundBallLose;
        playSound("ball_lose", 1, 0, oSoundBallLose);
    };
    ...
    this.bonusFireBall = function () {
        ...
        if (DISABLE_SOUND_MOBILE === false || s_bMobile === false && _oAudioFireBall === null) {
            _oAudioFireBall = createjs.Sound.play("fireball", {loop: -1});
        }
        this.audioPowerUpBonus();
    };
    ...
    this.stopFireSound = function () {
        if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
            _oAudioFireBall.destroy();
            _oAudioFireBall = null;
        }
    };
    ...
    this.audioBrickExplosion = function () {
        playSound("brick_explosion", 1, 0);
    };

    this.audioPowerUpBonus = function () {
        playSound("power_up_bonus", 1, 0);
    };

    this.audioPowerUpMalus = function () {
        playSound("power_up_malus", 1, 0);
    };

    this.audioBrickMetal = function () {
        playSound("brick_metal", 1, 0);
    };

    this.audioBullet = function () {
        playSound("bullet", 1, 0);
    };
    ...
    this.gestureBricks = function (aBrick, iID, iBrickX, iBrickY) {
        if (aBrick[iID].getInfoData("destructible") === true || _bFireBall === true) {
            ...
            else
            {
                ...
                var oSoundBrickCrack;
                playSound("brick_crack", 1, 0, oSoundBrickCrack);
            }
        } else {
