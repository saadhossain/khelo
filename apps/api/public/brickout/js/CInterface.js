function CInterface() {
    ...
    this.gameOver = function (iScore) {
        ...
        if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
            var oGameOver;
            oGameOver = createjs.Sound.play("game_over");//insert an appropriate sound
            s_oSoundTrack.volume = 0.04;
            oGameOver.on("complete", function () {
    ...
    this.finishGame = function (iScore) {
        ...
        if (DISABLE_SOUND_MOBILE === false || s_bMobile === false) {
            var oGameCompleted;
            oGameCompleted = createjs.Sound.play("game_completed");//insert an appropriate sound
            s_oSoundTrack.volume = 0.04;
            oGameCompleted.on("complete", function () {
                s_oSoundTrack.volume = 1;
                oGameCompleted = null;
            });
        }
