...
import { FaGamepad, FaRegClone } from "react-icons/fa";
...
const DisplaySingleGameForUser = ({ game }: { game: GamesDataType }) => {
    ...
    <div className="bg-secondary shadow-md rounded-md p-4 md:flex relative">
      ...
      <div className="w-full md:w-3/5 flex flex-col justify-between mb-5 md:mb-0">
        ...
        <div className="flex items-center gap-5">
          ...
          <Link
            to={`/editor/${game.storage_id}`}
            className={`flex gap-1 items-center py-1 px-2 rounded bg-accent text-white text-center font-semibold ${game.category === "default" && "hidden"}`}
          >
            <FaGamepad className="w-6 h-6" /> Edit
          </Link>
          <label
...
export default DisplaySingleGameForUser;
